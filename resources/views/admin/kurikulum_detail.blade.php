...
@section('content')
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        <div>
            <h2 class="font-bold text-2xl text-gray-800 flex items-center gap-2">
                <span class="text-blue-900">{{ $konsentrasi->nama_konsentrasi }}</span>
            </h2>
        ...
        <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-5 animate-fade-in-up">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <form action="{{ url('/admin/kurikulum/update/' . $konsentrasi->id) }}" method="POST">
                    @csrf
                    <button type="submit" class="btn btn-warning text-yellow-900 font-bold shadow-sm hover:bg-yellow-400 transition text-sm">
                        <i class="bi bi-stars"></i> AI Auto-Insert
                    </button>
                </form>
            </div>
            <div class="row g-3">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-4 py-3 border-b border-gray-100 bg-gray-50 d-flex justify-content-between align-items-center">
            ...
            @if($courses->count() > 1)
                <form action="{{ url('/admin/kurikulum/reset/' . $konsentrasi->id) }}" method="POST" onsubmit="return confirm('Reset ulang urutan dengan AI?')">
                    @csrf
                    <button type="submit" class="btn btn-sm btn-outline-secondary font-bold text-xs hover:bg-gray-200 hover:text-gray-800 border-gray-300">
                ...
                <tbody class="text-sm">
                    @forelse($courses as $mk)
                        <tr>
                            ...
                            <td>
                                @if($mk->dosen)
                                    <div class="d-flex align-items-center gap-2 p-1 bg-gray-50 rounded border border-gray-100 w-fit">
                                        <div class="w-6 h-6 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-[10px]">
                            ...
                            {{-- Kolom Deskripsi --}}
                            <td class="text-gray-500" style="max-width: 300px;">
                            ...
                            <td class="text-end px-4 text-nowrap">
                                <div class="d-flex justify-content-end gap-1">
                                    <button class="btn btn-sm btn-light text-blue-600 border rounded hover:bg-blue-50" 
                                        data-bs-toggle="modal" data-bs-target="#modalEdit{{ $mk->id }}">
                                        <i class="bi bi-pencil-square"></i>
                                    </button>
                                    
                                    {{-- HAPUS (Detach) --}}
                                    <form action="{{ url('/admin/mata-kuliah/'.$mk->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus MK ini dari Konsentrasi? (MK tetap ada di Bank Data)')">
                                        @csrf @method('DELETE')
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content rounded-xl border-0 shadow-lg">
                                    <div class="modal-header border-bottom-0 pb-0">
                                        <h5 class="modal-title font-bold">Edit Master MK</h5>
                                    ...
                                    <form action="{{ url('/admin/mata-kuliah/'.$mk->id) }}" method="POST">
                                        ...
                                        <div class="modal-body">
                                            <div class="alert alert-warning text-xs border-0 bg-yellow-50 text-yellow-800 mb-3">
                                                ...
                                                Perubahan data di sini akan berlaku untuk <strong>semua konsentrasi</strong> yang menggunakan MK ini.
                                            </div>
                                            <div class="mb-3">
                                        ...
                                        <div class="modal-footer border-top-0 pt-0">
                                            <button type="submit" class="btn btn-primary bg-blue-900 border-0 w-100 font-bold">Simpan Perubahan Master</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-5 text-gray-400">
                                <i class="bi bi-inbox text-2xl d-block mb-2 opacity-50"></i>
                                Belum ada kurikulum di konsentrasi ini.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div class="modal fade" id="modalTambahMK" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-xl border-0 shadow-lg">
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title font-bold">Tambah / Ambil Mata Kuliah</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form action="{{ url('/admin/mata-kuliah') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" name="nama_mk" class="form-control" placeholder="Contoh: Algoritma" required>
                            <div class="form-text text-xs text-blue-600">
                                <i class="bi bi-check-circle-fill"></i> Jika nama MK sama dengan yang ada di Bank Data, sistem otomatis mengambil data lama (Dosen & Deskripsi).
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="deskripsi" class="form-control" rows="3" placeholder="Jelaskan secara singkat..."></textarea>
                        </div>
                        <div class="mb-3">
                    ...
                    <div class="modal-footer border-top-0 pt-0">
                        ...
                        <button type="submit" class="btn btn-primary bg-blue-900 border-0 font-bold">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
